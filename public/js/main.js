...
const config = {
  ...
  function setupSocketHandlers() {
    ...
    s.on('connect',    () => updateUI('Connected', state.deviceRole));
    s.on('disconnect', () => updateUI('Disconnected', state.deviceRole));
    ...
    s.on('phone-reconnected', saved => {
      ...
      if (saved?.values) {
        ...
        if (state.targetN !== null) requestFibonacciComputation();
      } else {
